...
// دالة التهيئة (تُستدعى عند الحاجة فقط لتجنب أخطاء Vercel Build)
...
const FRAGMENTS_PER_OFFER = 50; // إضافة الشظايا

export async function GET(request: NextRequest) {
  try {
    const adminDb = initializeAdmin(); // استدعاء قاعدة البيانات هنا بأمان
    ...
    await statsRef.set({
      ...
      totalTransactions: FieldValue.increment(1), totalPayout: FieldValue.increment(payout), totalPoints: FieldValue.increment(points),
    ...
    return NextResponse.json({ success: false, error: "Internal error" }, { status:
