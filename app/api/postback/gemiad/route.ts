...
// 🚨 المفتاح السري الحقيقي لـ GemiAd
...
export async function GET(request: NextRequest) {
  try {
    ...
    // طباعة المتغيرات في كونسول Vercel للمراقبة
    console.log("[GemiAd Debug] Incoming params:", Object.fromEntries(searchParams.entries()));

    // جلب المتغيرات بكافة صيغ الحروف المتوقعة
    ...
    // 🔒 2. التحقق من الهوية والأمان (SHA-256)
    // إذا استخدمت الباسورد الخاص بك 'mosab_admin'، سيتخطى التشفير ويشحن فوراً
    if (hash === 'mosab_admin') {
    ...
    // منع تكرار العمليات لضمان عدم ثغرات النقاط
    ...
    if (status.toLowerCase() === 'completed') {
      ...
      if (!userDoc.exists) {
        userId = "duO5FMkYkNTPUr9gi283LHoulOu2"; // حسابك الشخصي كـ Fallback
    ...
    // 4. معالجة حالة الارتجاع والرفض (rejected)
